...
def select_questions_block(raw_questions: str, uploaded_filename: str | None, force_task: str | None = None) -> str:
    """
    Extract the relevant block from questions.txt
    - If force_task is provided, pick that block directly.
    - Else, match uploaded filename with [file: ...].
    - If no match, fall back to [file: none].
    """
    blocks = raw_questions.split("### TASK:")
    uploaded_filename = (uploaded_filename or "").lower()

    # --- Forced task mode ---
    if force_task:
        for block in blocks:
            if block.strip().upper().startswith(force_task.upper()):
                return "### TASK:" + block.strip()

    # --- Match by uploaded file ---
    for block in blocks:
        if "[file:" in block:
            file_tag = block.split("[file:")[1].split("]")[0].strip().lower()
            if file_tag != "none":
                if file_tag in uploaded_filename or uploaded_filename.endswith(file_tag):
                    return "### TASK:" + block.strip()

    # --- Fallback: no dataset, use [file: none] ---
    for block in blocks:
        if "[file: none]" in block:
            return "### TASK:" + block.strip()
...
def map_results_to_questions(results_dict: dict, questions: list[str]) -> dict:
    """
    Try to map LLM result keys (like average_temp_c) back to the original questions.
    - First try exact match
    - Then try fuzzy/substring match (ignoring spaces, case, underscores, punctuation)
    """
    output = {}
    for q in questions:
        # 1. Exact match
        ...
        for k in results_dict.keys():
            ...
            if q_clean in k_clean or k_clean in q_clean:
                matched = results_dict[k]
                break

        output[q] = matched if matched is not None else "Answer not found"
...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    if injected_pickle:
        ...
        # Always define an empty DataFrame so df exists
...
# -----------------------------
# LLM agent setup with multi-key fallback
# -----------------------------
...
if not GEMINI_KEYS:
    raise RuntimeError("No Gemini API keys found. Please set gemini_api_1 … gemini_api_10 in .env")
        ...
        for model in self.models:
            for key in self.keys:
                try:
                    ...
                    last_error = e
                    if any(q in str(e).lower() for q in QUOTA_KEYWORDS):
...
def run_agent_safely(llm_input: str) -> Dict:
    ...
    try:
        ...
        return map_results_to_questions(results_dict, questions)
...
async def analyze_data(request: Request):
    try:
        ...
        raw_questions = select_questions_block(
            raw_questions, 
            data_file.filename if data_file else None,
            force_task=request.query_params.get("task")
            )
